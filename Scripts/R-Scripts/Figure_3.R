...
labels <- single_profile[,1:26] # Metadata
features <- single_profile[27:ncol(single_profile)] #Features
...
geomx_data <- geomx_data %>% select(-S6, -GAPDH, -`Histone H3`, -`Myelin basic protein`, -NeuN, -MAP2, -C4B, -Vimentin, -EMP1, 
                                    -`Neurofilament light`, -GFAP, -Synaptophysin, -S100B, -Olig2)#Remove Housekeepers
...
Cross_validation <- lda(x = single_profile[28:ncol(single_profile)], grouping = single_profile$Cluster, CV = TRUE) #Change according to number of metadata
...
total_cells_per_image <- single_profile %>%
  group_by(ImageNumber, Metadata_Sample_number, Slide, Patient, Sex, Age, Formalinfixed, Postdelay,
           Metadata_Pt_type, Metadata_Cortex_region, Metadata_Layer, FileName_ROI_Mask)%>% 
  count() %>% mutate(FileName_ROI_Mask = substr(FileName_ROI_Mask, 6, nchar(FileName_ROI_Mask) - 4)) #Cluster Object Count
total_cells_per_image <- total_cells_per_image %>% rename(n_cells_in_image = n) #rename "n_total" column
total_cells_per_image <- total_cells_per_image %>% rename(Region = FileName_ROI_Mask) #rename region ID column
...
total_clusters_per_image <- total_clusters_per_image %>% rename(n_clustercells_in_image = n) #rename n_cluster column
...
total_cells_per_group <- total_cells_per_group %>% rename(n_total_cell_count = n) #Rename "n_total" column
...
grouped_cluster_count  <- grouped_cluster_count  %>% rename(n_cluster_in_group = n) #Rename "n_cluster" column
...
# Extra - Variability per patient
total_cells_per_patient <- single_profile %>%
  group_by(Patient, Metadata_Pt_type, Metadata_Cortex_region, Metadata_Layer) %>%
  count() %>%
  rename(n_total_cell_count = n)

# Cluster counts per patient
grouped_cluster_count <- single_profile %>%
  group_by(Patient, Metadata_Pt_type, Metadata_Cortex_region, Metadata_Layer, Cluster) %>%
  count() %>%
  rename(n_cluster_in_group = n)

# Join and compute percentages
cluster_distribution <- left_join(
  grouped_cluster_count,
  total_cells_per_patient,
  by = c("Patient","Metadata_Pt_type","Metadata_Cortex_region","Metadata_Layer")
) %>%
  mutate(percentage = (n_cluster_in_group / n_total_cell_count) * 100)

# Reorder if desired
cluster_distribution$Metadata_Pt_type <- factor(cluster_distribution$Metadata_Pt_type,
                                                levels = c("CTRL", "AD", "CJD"))
cluster_distribution$Metadata_Layer <- factor(cluster_distribution$Metadata_Layer,
                                              levels = c("sGM","dGM","WM"))
cluster_distribution <- cluster_distribution %>%
  mutate(
    Patient = factor(
      Patient,
      levels = paste("Patient", 1:12)))

# Stacked bar chart: patient bars, grouped by disease group
p_stacked <- ggplot(cluster_distribution,
       aes(x = Patient, fill = Cluster, y = percentage)) +
  ggh4x::facet_nested(
    rows = vars(Metadata_Layer),                             # layers as rows
    cols = vars(Metadata_Pt_type, Metadata_Cortex_region),    # nested columns
    scales = "free_x",
    space  = "free_x" 
  ) +
...
count_per_image <- single_profile %>% group_by(ImageNumber, Cluster) %>% count(name = "cluster_count") # Total Object count
count_per_image <- count_per_image %>%
  left_join(total_cells_per_image %>% select(ImageNumber, n_cells_in_image), by = "ImageNumber") %>%
  mutate(cluster_percentage = (cluster_count / n_cells_in_image) * 100) #Add cluster data, calculate percentage

# Pivot to wide format (one row per image, columns per cluster percentage)
cluster_wide <- count_per_image %>%
  select(ImageNumber, Cluster, cluster_percentage) %>%
  pivot_wider(names_from = Cluster, values_from = cluster_percentage, names_prefix = "cluster_pct_") %>%
  replace(is.na(.), 0)
cluster_wide <- cluster_wide %>%
  left_join(total_cells_per_image %>% select(ImageNumber, Region), by = "ImageNumber")

#Join GeoMx and single object data
combined_data <- geomx_data %>%
  rename(`ROI (Label)` = `ROI  Label `) %>%
  left_join(cluster_wide %>% rename(`ROI (Label)` = Region), by = c("ROI (Label)", "Patient"))

#Rename Column names
combined_data <- combined_data %>% rename("IBA1+ Object Cluster 1 Proportion" = cluster_pct_1,
                                          "IBA1+ Object Cluster 2.1 Proportion" = cluster_pct_2,
                                          "IBA1+ Object Cluster 2.2 Proportion" = cluster_pct_3)
...
#Extra - evaluating effect of Age, Sex, formalin fixation durationa and postmortem delay on cluster densities
covariates <- c("Age", "Sex", "Formalinfixed", "Postdelay")

statistics_covariate_effects <- total_clusters_per_image %>%
  group_by(Cluster) %>%
  group_modify(~ {
    cluster_data <- .x
    current_cluster <- .y$Cluster
    
    print(paste("Checking covariate effects for Cluster", current_cluster))
    
    results_list <- list()
    
    for (covariate in covariates) {
      formula_str <- paste0("cell_per_area ~ ", covariate, " + (1 | Patient/Metadata_Sample_number)")
      model_formula <- as.formula(formula_str)
      
      model <- glmmTMB(model_formula, family = nbinom1(link = "log"), data = cluster_data)
      
      summary_model <- summary(model)
      covariate_effect <- summary_model$coefficients$cond
      
      # Extract row related to covariate (for factors like Sex, might have multiple rows)
      covariate_rows <- grep(covariate, rownames(covariate_effect), fixed = TRUE)
      
      effect_summary <- covariate_effect[covariate_rows, , drop = FALSE]
      effect_summary <- as.data.frame(effect_summary)
      effect_summary$Covariate <- covariate
      
      results_list[[covariate]] <- effect_summary
    }
    
    # Combine all covariate effects for this cluster
    cluster_results <- bind_rows(results_list)
    return(cluster_results)
  }) %>%
  ungroup()

print(statistics_covariate_effects)
