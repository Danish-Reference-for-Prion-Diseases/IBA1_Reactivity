...
###########DATA##############
##Working Directory
#wd <- here("Figures and Tables", "Figure 4")
#setwd(here("Figures and Tables", "Figure 4"))
...
loading_with_labels <- loading + 
  ...
  scale_colour_gradientn(colours = c("#00AFBB", "#FC4E07"), 
                         ...
                         breaks = c(0, 5, 10))
loading_with_labels
#ggsave("loading1+3PCA.png", path = wd, width = 8, height = 8, device='png', dpi=300)
...
proteins_var <- plotPercentBars(varPart[1:73, ], 
                                col =varCol) + theme(
...
#ggsave("Single Features/MaximumRadius.png", width = 4.5, height = 3.5, device="png", dpi = 300)

#Plot - Feature Mean Radius
interaction_plot("Total_AreaShape_MeanRadius", "Total - Mean Radius")
ggsave("Single Features/MeanRadius.png", width = 4.5, height = 3.5, device="png", dpi = 300)
...
ggsave("Single Features/Textcorrelation.png", width = 7, height = 3.5, device="png", dpi = 300)
...
#ggsave("MOFA_variance_explained.png", path=wd, width = 5.2, height = 3.5, dpi = 300) #Save
...
#Save
#png("MOFA/featureloading_factor1.png", width = 5.5, height = 4, res = 300, units = "in")
#grid.arrange(grobs = aligned_plots, nrow = 2, ncol = 1)
...
ggsave("MOFA/factor1_view1.png", width = 9.5, height = 5, dpi = 300)


plot_data_scatter(model, view = "view_2", #Change view
                  factor=2, #Change Factor
                  features=5,
                  add_lm=T,
                  color_by="Disease Group",
                  shape_by = "Cortex Layer")

ggsave("MOFA/factor1_view2.png", width = 9.5, height = 5, dpi = 300)
...
ggsave("MOFA/violin_factor1.png", width = 8, height = 4, dpi = 300)


#Generate new list isolating factor 2 for proteins
plot_top_weights(model, view = views[1], factor = 2, nfeatures = 5) +
  theme(text = element_text(size = 12),
        plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "lines"))
#Save
png("MOFA/featureloading_factor2Proteins.png", width = 4, height = 3, res = 300, units = "in")
plot_top_weights(model, view = views[1], factor = 2, nfeatures = 5) +
  theme(text = element_text(size = 12),
        plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "lines"))
...
draw_trajectory_plot(space, Interest_tract, traj$path, contour = TRUE, progression_group_palette = disease_color)+
  ...
  guides(shape = guide_legend(title = "Brain Region"), 
         ...
         fill="none") # Plot
#ggsave("SCORPIUS/Trajectory.png", width = 5.5, height = 4, device='png', dpi=300)#Save

traj <- reverse_trajectory(traj)#Optional: May need to reverse Trajectory for correct heatmap plot
...
png("SCORPIUS/Scorpiusheat.png", width = 15, height=6, units = "in", res = 300)
draw(ht, heatmap_legend_side="bottom", annotation_legend_side="right", legend_grouping = "original")
dev.off()
