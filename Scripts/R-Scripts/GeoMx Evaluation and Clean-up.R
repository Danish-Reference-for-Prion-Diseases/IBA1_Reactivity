...
ROI_boxplots <- function(dataset, name){
  ggplot(dataset, aes(x = Sample_ID, y = count, fill = Slide)) +
    geom_boxplot(size = 1) +  # Adjust boxplot line size if needed
    labs(x = "Sample ID",             # Label for the x-axis
         y = "Log2 Protein Count",       # Label for the y-axis
         title = name) +     # Title for the plot
    ...
    theme(
      ...
      panel.grid.major = element_line(size = 0.5),  # Adjust grid line size (optional)
      panel.grid.minor = element_line(size = 0.25),  # Adjust minor grid line size (optional)
      plot.margin = margin(10, 10, 10, 10)   # Adjust plot margin if labels are cut off
    )
}


ROI_density <- function(dataset, name){
  ggplot(dataset, aes(x = count, fill = Sample_ID)) +  # Plot 'count' on x-axis
    geom_density(alpha = 0.25) +  # Add density plot with transparency (alpha=0.6)
    labs(x = "Log2 Protein Count",             # Label for the x-axis
...
#####EVALUATION#######################################################
#setwd(here("Figures and Tables", "GeoMx Evaluation")) # output folder
...
#House keepers
#png("histogram_geomean_housekeepers.png", width = 900, height = 600, res = 300)
hist(Data_Matrix_evaluate$geomean_hk, xlab ="AOIs' log2 HK Geomean (Counts)", main=NULL,  xlim = c(0, 14))
#dev.off()

#Negative Controls
#png("histogram_geomean_negativeControls.png", width = 900, height = 600, res = 300)
hist(Data_Matrix_evaluate$geomean_negative, xlab ="AOIs' log2 IgG Geomean (Counts)", main=NULL, xlim = c(0,7))
#dev.off()


###Geomean per AOI
#png("GeomeanHK_per_AOI.png", width = 900, height = 600, res = 300)
...
#png("Geomeannegative_per_AOI.png", width = 900, height = 600, res = 300)
...
#HK vs Geomean
#png("correlationsofcounts.png", width = 900, height = 600, res = 300)
...
#HK genes
s6_norm <- normalization_visualization('S6')
histone_h3_norm <- normalization_visualization('Histone H3')
gapdh_norm <- normalization_visualization('GAPDH')

grid.arrange(s6_norm, histone_h3_norm, gapdh_norm, nrow = 1, ncol = 3)  
#ggsave("housekeeper_norm_tendencies.png", plot = grid.arrange(s6_norm, histone_h3_norm, gapdh_norm, nrow = 1, ncol = 3) , width = 24, height = 6, dpi=300)

#Negative Controls
rb_igG <- normalization_visualization('Rb IgG')
msigg2 <- normalization_visualization('Ms IgG2a')
msigg1 <- normalization_visualization('Ms IgG1')

grid.arrange(rb_igG, msigg2, msigg1, nrow = 1, ncol = 3)  
#ggsave("negative_norm_tendencies.png", plot = grid.arrange(rb_igG, msigg2, msigg1, nrow = 1, ncol = 3) , width = 24, height = 6, dpi=100)

#Area & Nuclei
Data_Matrix_evaluate$`AOI surface area` <- as.numeric(Data_Matrix_evaluate$`AOI surface area`)
area <- normalization_visualization('AOI surface area')
Data_Matrix_evaluate$`AOI nuclei count` <- as.numeric(Data_Matrix_evaluate$`AOI nuclei count`)
nuclei_count <- normalization_visualization("AOI nuclei count")

grid.arrange(area, nuclei_count, nrow = 1, ncol = 2)  
#ggsave("shape_norm_tendencies.png", plot =grid.arrange(area, nuclei_count, nrow = 1, ncol = 2)  , width = 16, height = 6, dpi=100)


###############Evaluate Final Normalization
#ALL To long format
#Evaluation Data
...
#ggsave("normalizationeffect_boxplots.png",  plot = grid.arrange(raw, hk_normalized, negative_normalized, area_normalized, nrow = 2, ncol = 2) , width = 16, height = 12, dpi = 100)
...
#ggsave("normalizationeffect_DensityPlots.png", plot = grid.arrange(raw, hk_normalized, negative_normalized, area_normalized, nrow = 2, ncol = 2) , width = 16, height = 12, dpi =100)




#######
#From Evaluation, Write the datamatrix
