...
####Datasets----------------------------------------------------------------------------
#Working Directory
#wd <- here("Figures and Tables", "Figure 2")
#setwd(here("Figures and Tables", "Figure 2"))
...
#ggsave("clusterUMAP.png", path = wd, width = 5, height = 4, device='png', dpi=300)
...
#Visualize and Save
#dev.new()
#png(here(wd, "Extra", "boruta.png"), width = 10, height=6, units = "in", res = 300)
par(mar = c(17,2,2,2))
...
htmp
#dev.new()
#png("Extra/heatmapfinalclust.png", width = 10, height=6, units = "in", res = 600)
...
#dev.new()
#png("Radar_Charts.png", width = 12.4, height=3.75, units = "in", res = 300)
...
#Select random IBA1+ Objects for Visualization (Look-up in Cropped Objects Folder from Cellprofiler)
single_profile %>% filter(Cluster==1) %>% sample_n(3)
single_profile %>% filter(Cluster==2) %>% sample_n(3)
single_profile %>% filter(Cluster==3) %>% sample_n(3)
...
single_profile <- read.csv(file.path(maindir, "SingleMicroglia_profile_with_cluster.csv"))
single_profile$Cluster <- as.factor(single_profile$Cluster)
...
#ggsave("Objectclusterdistribution.png", path = wd, width = 6, height = 5, device='png', dpi=300)

###2. Stacked Bar Chart of Distribution of the total object population
#Prepare data
...
p_stacked <- ggplot(cluster_distribution, aes(x=Metadata_Pt_type, fill = Cluster, y=percentage))+
  ...
  theme(text = element_text(size = 10, face="bold"),strip.text = element_text(face = "bold"),
        ...
        legend.title.position = "top")
p_stacked
#ggsave("Stacked_chart.png", path = wd, width = 4, height = 5, device='png', dpi=300)
...
#ggsave("Objectclustercorrelation_new.svg", path = wd, width = 10, height = 6, device='svg', dpi=300)
