...
protein_list <- read_excel(file.path("C:/Users/VTKA0002/OneDrive - Region Hovedstaden/PhD/Articles/First Article - Regional Microglial Reactivity/Data/GeoMx (protein)/Protein_panels_list.xlsx"))


#Saving Directory
#directory <- here("Figures and Tables")
...
#gt_tbl %>% gt::gtsave(filename = "Table1_Patientdata.docx", path=directory)
...
protein_list %>%
  ...
  )#%>% gt::gtsave(filename = "Table 2_Protein_Morphology_List.docx", path=directory)
...
#ggsave("sankey_plot_subsampling.png", path = directory, height=6, width = 10.5, device="png", dpi=300)
